...
ESolider::ESolider()
{
	...
	m_EUnitName = ENEMY_UNIT_NAME::SOLIDER;
	...
	desc.FPS = 5;
	desc.FrmCount = 4;
	desc.pAtlas = CAssetMgr::Get()->LoadTexture(L"eSolider", L"texture\\enemy\\enemy_sodlier.png");
...
ESolider::ESolider(const ESolider& _Other)
	:CUnit(_Other)
{
	m_Animator->Play(L"ESolider_Move", true);
}
...
void ESolider::PlayAnimation(const wstring& _Name, bool _Repeat)
{
	wstring str = L"ESolider_" + _Name;
	m_Animator->Play(str, _Repeat);
}

void ESolider::ShootBullet(Vec2 _Dir)
{
...
void ESolider::Demagecall(Unit_Stat& _EnemyStat, CObj* _Other)
{
