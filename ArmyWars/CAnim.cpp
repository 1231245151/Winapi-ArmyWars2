...
void CAnim::Render()
{
	...
	if (m_Owner->IsRenderPos())
	{
		vPos = m_Owner->GetOwner()->GetRenderPos();
	}
	else
	{
		vPos = m_Owner->GetOwner()->GetFinalPos();
	}
	Vec2 vFinalPos = vPos + m_Owner->GetOffset();
	...
	// 알파 블랜딩 랜더링
	AlphaBlend(BackDC, vFinalPos.x - m_vecFrm[m_CurIdx].vSlice.x / 2.f + m_vecFrm[m_CurIdx].vOffset.x,
		vFinalPos.y - m_vecFrm[m_CurIdx].vSlice.y / 2.f + m_vecFrm[m_CurIdx].vOffset.y,
		m_vecFrm[m_CurIdx].vSlice.x, m_vecFrm[m_CurIdx].vSlice.y,
		m_Atlas->GetDC(),
		m_vecFrm[m_CurIdx].vLeftTop.x, m_vecFrm[m_CurIdx].vLeftTop.y,
		m_vecFrm[m_CurIdx].vSlice.x, m_vecFrm[m_CurIdx].vSlice.y
