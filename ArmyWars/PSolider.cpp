...
PSolider::PSolider()
{
	...
	m_PUnitName = PLAYER_UNIT_NAME::SOLIDER;
	...
	desc.FPS = 5;
	desc.FrmCount = 4;
	desc.pAtlas = CAssetMgr::Get()->LoadTexture(L"pSolider", L"texture\\player\\player_solider.png");
...
PSolider::PSolider(const PSolider& _Other)
	:CUnit(_Other)
{
	m_Animator->Play(L"PSolider_Move", true);
}
...
void PSolider::PlayAnimation(const wstring& _Name, bool _Repeat)
{
	wstring str = L"PSolider_" + _Name;
	m_Animator->Play(str, _Repeat);
}

void PSolider::ShootBullet(Vec2 _Dir)
{
...
void PSolider::Demagecall(Unit_Stat& _EnemyStat, CObj* _Other)
{
