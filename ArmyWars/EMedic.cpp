...
EMedic::EMedic()
{
	...
	// 유닛 스텟 설정
	GetStat().Armor = 10;
	GetStat().AttackDuration = 2.f;
	GetStat().AttackSpeed = 2.f;
	...
	m_EUnitName = ENEMY_UNIT_NAME::MEDIC;
	...
	// Animation 설정
	tAnimDesc desc = {};
	desc.AnimName = L"EMedic_Move";
	desc.FPS = 5;
	desc.FrmCount = 4;
	desc.pAtlas = CAssetMgr::Get()->LoadTexture(L"pEMedic", L"texture\\enemy\\enemy_medic.png");
	desc.SliceSize = Vec2(93.f, 63.f);
	desc.StartLeftTop = Vec2(0.f, 0.f);
	m_Animator->CreateAnimation(desc);

	desc.AnimName = L"EMedic_Death";
	desc.FPS = 5;
	desc.FrmCount = 4;
	desc.StartLeftTop = Vec2(372.f, 0.f);
	m_Animator->CreateAnimation(desc);

	desc.AnimName = L"EMedic_Stend";
	desc.FPS = 5;
	desc.FrmCount = 3;
	...
	m_Animator->Play(L"EMedic_Move", true);
...
EMedic::EMedic(const EMedic& _Other)
	:CUnit(_Other)
{
	m_Animator->Play(L"EMedic_Move", true);
}
...
void EMedic::PlayAnimation(const wstring& _Name, bool _Repeat)
{
	wstring str = L"EMedic_" + _Name;
	m_Animator->Play(str, _Repeat);
}

void EMedic::Demagecall(Unit_Stat& _EnemyStat, CObj* _Other)
{
