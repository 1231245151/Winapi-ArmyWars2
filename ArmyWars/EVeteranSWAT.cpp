...
EVeteranSWAT::EVeteranSWAT()
{
	...
	GetStat().velocity = Vec2(-140.f, 0.f);


	SetScale(Vec2(30.f, 30.f));
	SetShootType(UNIT_ATTACK::INSTANCE);

	m_EUnitName = ENEMY_UNIT_NAME::VETERANSWAT;
	...
	desc.FPS = 5;
	desc.FrmCount = 4;
	desc.pAtlas = CAssetMgr::Get()->LoadTexture(L"EVeteranSWAT", L"texture\\enemy\\enemy_veteranswat.png");
...
EVeteranSWAT::EVeteranSWAT(const EVeteranSWAT& _Other)
	:CUnit(_Other)
{
	m_Animator->Play(L"ESWAT_Move", true);
}
...
void EVeteranSWAT::PlayAnimation(const wstring& _Name, bool _Repeat)
{
	wstring str = L"ESWAT_" + _Name;
	m_Animator->Play(str, _Repeat);
}

void EVeteranSWAT::Demagecall(Unit_Stat& _EnemyStat, CObj* _Other)
{
