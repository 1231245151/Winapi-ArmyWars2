...
PVeteranSWAT::PVeteranSWAT()
{
	SetName(L"베테랑 기동대원");
	// 유닛 스텟 설정
	GetStat().Armor = 10;
	GetStat().AttackDuration = 1.f;
	GetStat().AttackSpeed = 1.f;
	GetStat().Cooltime = 30.f;
	...
	GetStat().HDemage = 137;
	GetStat().MDemage = 173;
	GetStat().Range = Vec2(100.f, 100.f);
	GetStat().Type = UNIT_TYPE::HUMEN;
	GetStat().velocity = Vec2(150.f, 0.f);

	m_PUnitName = PLAYER_UNIT_NAME::VETERANSWAT;
	...
	desc.FPS = 5;
	desc.FrmCount = 4;
	desc.pAtlas = CAssetMgr::Get()->LoadTexture(L"pVeteranPSWAT", L"texture\\player\\player_veteranswat.png");
...
PVeteranSWAT::PVeteranSWAT(const PVeteranSWAT& _Other)
	:CUnit(_Other)
{
	m_Animator->Play(L"PSWAT_Move", true);
}
...
void PVeteranSWAT::PlayAnimation(const wstring& _Name, bool _Repeat)
{
	wstring str = L"PSWAT_" + _Name;
	m_Animator->Play(str, _Repeat);
}

void PVeteranSWAT::Demagecall(Unit_Stat& _EnemyStat, CObj* _Other)
{
...
void PVeteranSWAT::PlayDeathSound()
{
	CSound* pvoice = CAssetMgr::Get()->LoadSound(L"Women2Voice", L"sound\\death_women2.wav");
	pvoice->Play();
}

void PVeteranSWAT::PlayShootSound()
{
