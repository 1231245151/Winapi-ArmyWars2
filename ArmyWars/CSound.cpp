...
{
	if (nullptr == CSoundMgr::Get()->GetSoundDevice())
		assert(nullptr);
...
bool CSound::LoadWaveSound(const wstring& _strPath)
{
	...
	if (FAILED(CSoundMgr::Get()->GetSoundDevice()->CreateSoundBuffer(&m_tBuffInfo, &m_pSoundBuffer, NULL)))
	{
...
void CSound::PlayToBGM(bool _bLoop, bool _stop)
{
	CSoundMgr::Get()->RegisterToBGM(this, _stop);
